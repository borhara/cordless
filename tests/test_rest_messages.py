...
def test_fetch_channel_messages_returns_message_list():
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse([_MESSAGE_PAYLOAD])]) as urlopen:
...
def test_fetch_poll_answer_voters_returns_user_list():
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse({"users": [_USER_PAYLOAD]})]) as urlopen:
...
def test_fetch_poll_answer_voters_passes_after_and_limit():
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse({"users": []})]) as urlopen:
...
def test_bot_send_message_returns_the_sent_message():
    bot = Cordless()
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse(_MESSAGE_PAYLOAD)]):
...
def test_bot_fetch_poll_answer_voters_delegates_to_rest_module():
    bot = Cordless()
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse({"users": [_USER_PAYLOAD]})]):
