...
def test_fetch_guild_member_returns_member_with_guild_id_injected():
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse(_MEMBER_PAYLOAD)]) as urlopen:
...
def test_fetch_guild_roles_returns_role_list():
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse([_ROLE_PAYLOAD])]) as urlopen:
...
def test_bot_fetch_guild_member_delegates_to_rest_module():
    bot = Cordless()
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse(_MEMBER_PAYLOAD)]):
...
def test_bot_fetch_guild_roles_delegates_to_rest_module():
    bot = Cordless()
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse([_ROLE_PAYLOAD])]):
...
def test_bot_fetch_guild_role_member_counts_delegates_to_rest_module():
    bot = Cordless()
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse({"1": 5})]):
...
def test_guild_fetch_role_member_counts_delegates_to_rest_module():
    guild = Guild({"id": "10"})
    with patch.dict(os.environ, BOT_ENV), send_patch([FakeDiscordResponse({"1": 9})]):
